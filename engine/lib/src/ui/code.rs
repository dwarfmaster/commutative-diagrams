use crate::runtime::Runtime;
use crate::ui::VM;
use crate::vm;
use itertools::Itertools;

pub fn code<RT: Runtime>(ctx: &egui::Context, rt: &mut RT, vm: &mut VM<RT::Rem>) {
    if let Some(code) = &mut vm.code.try_lock() {
        if code.code_window_open {
            let mut open = code.code_window_open;
            egui::Window::new("Script")
                .open(&mut open)
                .show(ctx, |ui| code_impl(ui, rt, vm));
            code.code_window_open = open;
        }
    }
}

fn code_impl<RT: Runtime>(ui: &mut egui::Ui, rt: &mut RT, vm: &mut VM<RT::Rem>) {
    ui.with_layout(egui::Layout::top_down_justified(egui::Align::RIGHT), |ui| {
        code_text_box(ui, rt, vm, 150.0, true);
        egui::ScrollArea::vertical()
            .max_height(100.0)
            .id_source("error_msg_scroll_area")
            .show(ui, |ui| {
                let mut msg = if let Some(code) = vm.code.try_lock() {
                    code.error_msg.clone()
                } else {
                    "".to_string()
                };
                ui.add_sized(
                    ui.available_size(),
                    egui::TextEdit::multiline(&mut msg)
                        .font(egui::TextStyle::Monospace)
                        .interactive(false),
                )
            });
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            let running = rt.running();
            ui.add_enabled_ui(running.is_none(), |ui| {
                if ui.add(egui::Button::new("Run")).clicked() {
                    log::debug!("Running");
                    rt.run("Run code", move |vm| async {
                        if let Some(ast) = vm.recompile().await {
                            vm.run(ast).await;
                        }
                    });
                }
                if ui.add(egui::Button::new("Check")).clicked() {
                    log::debug!("Recompiling");
                    rt.run("Recompile code", move |vm| async {
                        vm.recompile().await;
                    });
                }
            });
        });
    });
}

pub fn code_text_box<RT: Runtime>(ui: &mut egui::Ui, rt: &mut RT, vm: &mut VM<RT::Rem>, height: f32, format: bool) {
    if let Some(code) = &mut vm.code.try_lock() {
        let format_none = egui::TextFormat {
            font_id: egui::FontId::monospace(14.0),
            color: ui.style().noninteractive().fg_stroke.color,
            background: ui.style().visuals.extreme_bg_color,
            italics: false,
            underline: egui::Stroke::NONE,
            strikethrough: egui::Stroke::NONE,
            valign: egui::Align::BOTTOM,
            ..Default::default()
        };
        let format_err = egui::TextFormat {
            underline: egui::Stroke {
                color: ui.style().visuals.error_fg_color,
                width: ui.style().visuals.text_cursor.width,
            },
            ..format_none.clone()
        };
        let format_run = egui::TextFormat {
            background: ui.style().visuals.code_bg_color,
            ..format_none.clone()
        };

        let sections = if format {
            code
                .code_style
                .iter()
                .chain(std::iter::once(&(code.code.len(), vm::CodeStyle::None)))
                .tuple_windows()
                .map(|((start, style), (end, _))| egui::text::LayoutSection {
                    leading_space: 0.0,
                    byte_range: *start..*end,
                    format: match style {
                        vm::CodeStyle::None => format_none.clone(),
                        vm::CodeStyle::Error => format_err.clone(),
                        vm::CodeStyle::Run => format_run.clone(),
                    },
                })
                .collect::<Vec<_>>()
        } else {
            vec![egui::text::LayoutSection {
                leading_space: 0.0,
                byte_range: 0..code.code.len(),
                format: format_none,
            }]
        };

        let mut layouter = |ui: &egui::Ui, string: &str, _width: f32| {
            // TODO memoize to avoid to realloc on each frame
            let sections = sections
                .iter()
                .filter_map(|sec| {
                    if sec.byte_range.start >= string.len() {
                        None
                    } else if sec.byte_range.end > string.len() {
                        Some(egui::text::LayoutSection {
                            byte_range: sec.byte_range.start..string.len(),
                            ..sec.clone()
                        })
                    } else {
                        Some(sec.clone())
                    }
                })
                .collect();
            let job = egui::text::LayoutJob {
                text: string.to_string(),
                sections,
                wrap: egui::epaint::text::TextWrapping {
                    max_width: 300.0,
                    max_rows: 0,
                    break_anywhere: true,
                    overflow_character: Some('…'),
                },
                ..Default::default()
            };
            ui.fonts(|f| f.layout_job(job))
        };

        egui::ScrollArea::vertical()
            .auto_shrink([false, false])
            .max_height(ui.available_height() - height)
            .show(ui, |ui| {
                let running = rt.running();
                ui.add_enabled_ui(running.is_none(), |ui| {
                    let event = ui.add_sized(
                        ui.available_size(),
                        egui::TextEdit::multiline(&mut code.code)
                            .code_editor()
                            .cursor_at_end(true)
                            .layouter(&mut layouter),
                    );
                    if event.changed() {
                        rt.run("Sync code", move |vm| async {
                            let code = &mut vm.code.lock().await;
                            code.reset_style();
                            code.style_range(0..code.run_until, vm::CodeStyle::Run);
                            vm.sync_code().await;
                        });
                    }
                });
            });
    }
}
