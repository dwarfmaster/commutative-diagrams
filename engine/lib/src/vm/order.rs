...
impl<Rm: Remote, I: Interactive> VM<Rm, I> {
    pub fn init_face_order(&mut self) {
        let mut faces_ids = (0..self.graph.graph.faces.len()).collect::<Vec<_>>();
        faces_ids.sort_by(|id1, id2| {
            let st = cmp_status(
                self.graph.graph.faces[*id1].label.status,
                self.graph.graph.faces[*id2].label.status,
            );
            if st == Ordering::Equal {
                self.graph.graph.faces[*id1]
                    .label
                    .name
                    .cmp(&self.graph.graph.faces[*id2].label.name)
            } else {
                st
            }
        });
        ...
        let split = faces_ids
            .binary_search_by_key(&1, |id| {
                if self.graph.graph.faces[*id].label.status == FaceStatus::Hypothesis {
                    1
                } else {
                    0
                }
            ...
            .unwrap_or_else(|x| x);
        self.graph.face_hyps_order = faces_ids.split_off(split);
        self.graph.face_goal_order = faces_ids;
    }

    // Return Ok(id) if it is in goals, and Err(id) otherwise
    fn locate_face_order(&self, fce: usize) -> Result<usize, usize> {
        for id in 0..self.graph.face_goal_order.len() {
            if self.graph.face_goal_order[id] == fce {
                return Ok(id);
            }
        }
        for id in 0..self.graph.face_hyps_order.len() {
            if self.graph.face_hyps_order[id] == fce {
                return Err(id);
            }
        }
    ...
    pub fn order_new_face(&mut self, fce: usize) {
        ...
        if let Some(parent) = self.graph.graph.faces[fce].label.parent {
            let after = self.graph.graph.faces[parent]
                .label
                .children
                .iter()
                .copied()
            ...
            match parent_pos {
                Ok(pos) => {
                    ...
                    self.graph.face_goal_order.insert(pos, fce);
                }
                Err(pos) => {
                    let pos = after.unwrap_or(pos) + 1;
                    self.graph.face_hyps_order.insert(pos, fce);
                }
            }
            return;
        }
        ...
        if self.graph.graph.faces[fce].label.status == FaceStatus::Hypothesis {
            self.graph.face_hyps_order.push(fce);
            return;
        } else {
            self.graph.face_goal_order.push(fce);
            return;
        }
    }
    ...
    pub fn order_rm_face(&mut self, fce: usize) {
        ...
        match pos {
            Ok(pos) => self.graph.face_goal_order.remove(pos),
            Err(pos) => self.graph.face_hyps_order.remove(pos),
        };
    }
}
