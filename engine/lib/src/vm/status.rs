...
impl<Rm: Remote> Context<Rm> {
    pub fn compute_eq_status_impl(&mut self, eq: &Eq) -> EvarStatus {
            ...
            for (_, _, blk) in &slice.blocks {
                ...
                status = match &blk.data {
                    ...
                    Funct(f, eq) => merge_statuses(
                        status,
                        merge_statuses(self.get_stored_status(*f), self.compute_eq_status_impl(eq)),
                    ),
                    Split => status,
                }
            }
    ...
    pub fn compute_eq_status(&mut self, eq: &Eq) -> FaceStatus {
        from_estatus(self.compute_eq_status_impl(eq))
    }
}
