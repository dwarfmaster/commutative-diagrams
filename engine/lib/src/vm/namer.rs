...
impl GraphState {
    ...
    pub async fn name_compute_node<Rm: Remote>(&self, ctx: &mut Context<Rm>, nd: usize) -> String {
        let lbl = &self.graph.nodes[nd].2.label;
        if is_valid_name(lbl) && self.names.get(lbl).is_none() {
            return lbl.clone();
        }

        // Try to name it from the category
        let cat_name = ctx.get_stored_label(self.graph.nodes[nd].1).to_lowercase();
    ...
    pub async fn name_compute_morphism<Rm: Remote>(
        &self,
        _ctx: &mut Context<Rm>,
        src: usize,
        mph: usize,
        ...
        let src_name = if is_valid_name(&self.graph.nodes[src].2.label) {
            self.graph.nodes[src].2.label.clone()
        } else {
            "".to_string()
        };
        let dst = self.graph.edges[src][mph].0;
        let dst_name = if is_valid_name(&self.graph.nodes[dst].2.label) {
            self.graph.nodes[dst].2.label.clone()
        } else {
            "".to_string()
        };
    ...
    pub async fn name_compute_face<Rm: Remote>(
        &self,
        _ctx: &mut Context<Rm>,
        base: &str,
        fce: usize,
...
impl<Rm: Remote, I: Interactive> VM<Rm, I> {
    ...
    pub async fn rename(&self, id: GraphId, name: &String) -> bool {
        use GraphId::*;
        ...
        let prev = {
            ...
            if let Some(oid) = graph.names.get(name) {
                return *oid == id;
            }
            graph.get_name(id)
        };
    ...
    pub async fn autoname_node(&self, nd: usize) {
        let graph = &mut self.graph.lock().await;
        assert!(graph.graph.nodes[nd].2.name.is_empty());
        let ctx = &mut self.ctx.lock().await;
        let name = graph.name_compute_node(ctx, nd).await;
        graph.set_name(GraphId::Node(nd), name.clone());
    }

    pub async fn autoname_morphism(&self, src: usize, mph: usize) {
        let graph = &mut self.graph.lock().await;
        assert!(graph.graph.edges[src][mph].1.name.is_empty());
        let ctx = &mut self.ctx.lock().await;
        let name = graph.name_compute_morphism(ctx, src, mph).await;
        graph.set_name(GraphId::Morphism(src, mph), name);
    }

    pub async fn autoname_face(&self, fce: usize) {
        let graph = &mut self.graph.lock().await;
        assert!(graph.graph.faces[fce].label.name.is_empty());
        let mut base = "Goal";

        // If the name of the parent is not empty (ie we're the first children
        // of the parent), and the parent is hidden, we copy the name. If the
        // parent isn't hidden, we use its name as base.
        if let Some(parent) = graph.graph.faces[fce].label.parent {
            if !graph.graph.faces[parent].label.name.is_empty() {
                if graph.graph.faces[parent].label.hidden {
                    ...
                    graph.set_name(GraphId::Face(parent), "".to_string());
                    graph.set_name(GraphId::Face(fce), parent_name);
                    return;
                } else {
        ...
        let ctx = &mut self.ctx.lock().await;
        let name = graph.name_compute_face(ctx, base, fce).await;
        graph.set_name(GraphId::Face(fce), name);
    }

    // To be called only once at the start.
    pub async fn autoname(&self) {
        let (nnodes, nmphs, nfaces) = {
            ...
            (
                ...
                graph
                    ...
                    .map(|v| v.len())
                    .collect::<Vec<_>>(),
                graph.graph.faces.len(),
            )
        };
        for n in 0..nnodes {
            self.autoname_node(n).await;
        }
        for src in 0..nnodes {
            for mph in 0..nmphs[src] {
                self.autoname_morphism(src, mph).await;
            }
        }
        for fce in 0..nfaces {
            self.autoname_face(fce).await;
        }
    }
}
