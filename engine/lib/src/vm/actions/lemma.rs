...
impl<Rm: Remote, I: Interactive> VM<Rm, I> {
    ...
    pub fn apply_lemma(&mut self, lemma: usize, matching: &[(GraphId, GraphId)]) -> bool {
        ...
        Self::lemma_extend_hash_matching(
            &matchings,
            &mut direct,
            &mut reverse,
            &pattern,
    ...
    pub fn lemma_extend_hash_matching(
        ...
        for pair in matching {
            use UnifyPair::*;
            match pair {
                Nodes(n1, n2) => Self::lemma_match_connect(
                    direct,
                    reverse,
                    GraphId::Node(*n1),
                    GraphId::Node(*n2),
                ...
                Morphisms((s1, m1), (s2, m2)) => Self::lemma_match_connect(
                    direct,
                    reverse,
                    GraphId::Morphism(*s1, *m1),
                    GraphId::Morphism(*s2, *m2),
                    pair,
                ),
                Faces(f1, f2) => Self::lemma_match_connect(
                    direct,
                    reverse,
                    GraphId::Face(*f1),
                    GraphId::Face(*f2),
    ...
    pub fn lemma_unify_matching(
        ...
        let get_value = |glem: &Graph, gvm: &Graph, pair: &UnifyPair| -> Option<(u64, u64)> {
            ...
            match pair {
                ...
                Morphisms((s1, m1), (s2, m2)) => {
                    Some((glem.edges[*s1][*m1].2, gvm.edges[*s2][*m2].2))
                }
                PathVM(_, _, _) => None,
    ...
    pub fn lemma_complete_matching(
        ...
        match (lem, goal) {
            ...
            (Morphism(lsrc, lmph), Morphism(gsrc, gmph)) => {
                ...
                matching.push(UnifyPair::Nodes(
                    pattern.edges[lsrc][lmph].0,
                    self.graph.graph.edges[gsrc][gmph].0,
                ));
            }
            (Face(lfce), Face(gfce)) => {
                matching.push(UnifyPair::Faces(lfce, gfce));
                matching.push(UnifyPair::Nodes(
                    pattern.faces[lfce].end,
                    self.graph.graph.faces[gfce].end,
                ));
                ...
                if pattern.faces[lfce].left.len() == self.graph.graph.faces[gfce].left.len() {
                    ...
                    matching.push(UnifyPair::PathVM(
                        (lsrc, pattern.faces[lfce].left[0]),
                        gsrc,
                        mph_gr,
                    ));
                } else if self.graph.graph.faces[gfce].left.len() == 1 {
                    let mph_lem = pattern.faces[lfce].left.clone();
                    matching.push(UnifyPair::PathLemma(
                        lsrc,
                        mph_lem,
                        (gsrc, self.graph.graph.faces[gfce].left[0]),
                    ));
                }
                ...
                if pattern.faces[lfce].right.len() == self.graph.graph.faces[gfce].right.len() {
                    ...
                    matching.push(UnifyPair::PathVM(
                        (lsrc, pattern.faces[lfce].right[0]),
                        gsrc,
                        mph_gr,
                    ));
                    ...
                    matching.push(UnifyPair::PathLemma(
                        lsrc,
                        mph_lem,
                        (gsrc, self.graph.graph.faces[gfce].right[0]),
                    ));
                }
            }
            _ => return false,
    ...
    fn lemma_connect(direct: &mut Mapping, lem: GraphId, pair: &UnifyPair) {
        if !direct.get(&lem).map(|v| v.contains(&pair)).unwrap_or(false) {
            direct.entry(lem).or_default().push(pair.clone());
        }
    }

    fn goal_connect(reverse: &mut Mapping, goal: GraphId, pair: &UnifyPair) {
        if !reverse
            .get(&goal)
            .map(|v| v.contains(&pair))
            .unwrap_or(false)
        {
