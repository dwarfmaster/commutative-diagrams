macro_rules! cat {
    ($ctx:expr, ?$e:expr) => {
        {
            let cat = $ctx.new_term("C".to_string(), None, crate::data::EvarStatus::Evar);
            $ctx.add_feat(cat, crate::data::Feature::Category);
            cat
        }
    };
    ($ctx:expr, (?$e:expr)) => {
    ...
    ($ctx:expr, :$e:expr) => {
        {
            let name = format!("C{}", $e);
            let cat = $ctx.new_term(name.as_str(), None, crate::data::EvarStatus::Grounded);
            $ctx.add_feat(cat, crate::data::Feature::Category);
            cat
        }
    };
...
macro_rules! funct {
    ...
    ($ctx:expr, ?$e:expr, $src:expr, $dst:expr) => {
        {
            let name = format!("F{}", $e);
            let funct = $ctx.new_term(name.as_str(), None, crate::data::EvarStatus::Evar);
            $ctx.add_feat(funct, crate::data::Feature::Functor { scat: $src, dcat: $dst });
            funct
        }
    };
    ($ctx:expr, (?$e:expr), $src:expr, $dst:expr) => {
    ...
    ($ctx:expr, :$e:expr, $src:expr, $dst:expr) => {
        {
            let name = format!("F{}", $e);
            let funct = $ctx.new_term(name.as_str(), None, crate::data::EvarStatus::Grounded);
            $ctx.add_feat(funct, crate::data::Feature::Functor { scat: $src, dcat: $dst });
            funct
        }
    };
...
macro_rules! obj {
    ...
    ($ctx:expr, ?$e:expr, $cat:expr) => {
        {
            let name = format!("x{}", $e);
            let obj = $ctx.new_term(name.as_str(), None, crate::data::EvarStatus::Evar);
            $ctx.add_feat(obj, crate::data::Feature::Object { cat: $cat });
            obj
        }
    };
    ($ctx:expr, (?$e:expr), $cat:expr) => {
    ...
    ($ctx:expr, :$e:expr, $cat:expr) => {
        {
            let name = format!("x{}", $e);
            let obj = $ctx.new_term(name.as_str(), None, crate::data::EvarStatus::Grounded);
            $ctx.add_feat(obj, crate::data::Feature::Object { cat: $cat });
            obj
        }
    };
