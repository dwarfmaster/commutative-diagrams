...
let build_evar ns env sigma tp =
...
let build_funct_obj ns env sigma objs =
...
let build_identity ns env sigma objs =
...
let build_funct_mph ns env sigma objs =
...
let build_reflexivity ns env sigma objs =
...
let build_concat ns env sigma objs =
...
let build_inverse_eq ns env sigma objs =
...
let build_compose_eq ns env sigma objs =
...
let build_assoc ns env sigma objs =
...
let build_right_application ns env sigma objs =
  let* [cat; src; mid; dst; mph; left; right; eq] = get_objs_value objs in
...
let build_funct_identity ns env sigma objs =
  let* [src; dst; funct; obj] = get_objs_value objs in
...
let build_funct_composition ns env sigma objs =
...
let build_funct_eq ns env sigma objs =
...
let build ns feat =
  ...
  match tag feat with
  | Category -> build_category ns env sigma objs
  | Object -> build_object ns env sigma objs
  | Morphism -> build_morphism ns env sigma objs
  | Functor -> build_functor ns env sigma objs
  | Equality -> build_eq ns env sigma objs
  | AppliedFunctObj -> build_funct_obj ns env sigma objs
  | Identity -> build_identity ns env sigma objs
  | ComposeMph -> build_compose_mph ns env sigma objs
  | AppliedFunctMph -> build_funct_mph ns env sigma objs
  | Reflexivity -> build_reflexivity ns env sigma objs
  | Concat -> build_concat ns env sigma objs
  | InverseEq -> build_inverse_eq ns env sigma objs
  | ComposeEq -> build_compose_eq ns env sigma objs
  | Associativity -> build_assoc ns env sigma objs
  | LeftUnitality -> build_left_unitality ns env sigma objs
  | RightUnitality -> build_right_unitality ns env sigma objs
  | LeftApplication -> build_left_application ns env sigma objs
  | RightApplication -> build_right_application ns env sigma objs
  | FunctIdentity -> build_funct_identity ns env sigma objs
  | FunctComposition -> build_funct_composition ns env sigma objs
  | AppliedFunctEq -> build_funct_eq ns env sigma objs
